...
# endif
    ...
    std::ofstream csvFile("heatmap1.csv", std::ios::app);
    if (!csvFile.is_open()) {
        NS_LOG_ERROR("Couldn't open the CSV file for writing.");
        pclose(pipe);
        return;
    }

    // Append the header to the CSV file
    csvFile << "Width,Length,Height,AverageDelay\n";
    ...
    while (!feof(pipe)) {
        if (fgets(buffer, 128, pipe) != nullptr) {
            if (sscanf(buffer, "Average delay: %f ms", &averageDelay) == 1) {
                // Line contains "Average delay", write values to the CSV file
                csvFile << width << "," << length << "," << height << "," << averageDelay << "\n";
            }
        }
    }
...
int main (int argc, char* argv[]) {
    ...
    for (const auto &dim : dimensions) {
        ...
        for (int twistIndex = 0; twistIndex < 64; ++twistIndex) {
            // Iterate through dimensions
                runSimulation(argc, argv, twistIndex);
            }
    }
    return 0;
}