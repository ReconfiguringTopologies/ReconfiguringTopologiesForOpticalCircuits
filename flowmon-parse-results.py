    ...
    for sim in sim_list:
        for flow in sim.flows:
            ...
            if flow.delayMean is None:
                ...
                sum_delay += flow.delayMean*1e3;
                max_delay = max(max_delay, flow.delayMean*1e3);
            #if flow.packetLossRatio is None:
                # print("\tPacket Loss Ratio: None")
            # else:
                # print("\tPacket Loss Ratio: %.2f %%" % (flow.packetLossRatio*100))

    print("Average delay: %.2f ms" % (1.0*sum_delay/count));
    print("Maximum delay: %.2f ms" % max_delay);
